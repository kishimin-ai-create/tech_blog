...
`ERROR-02`は一度Skippedの計画として分離した。その後、存在しない公開生成経路を通常のUnit testで証明しない方針をADR 0019として確定し、Skippedメソッド自体を削除した。一方、現在の公開値は通常のUnit testで明確に確認できるため、`REASON-01`として実装した。
...
直接assertionのテストには、新しいIDとして`REASON-01`を付けた。元の`ERROR-02`は、未定義値の生成拒否という別の契約を表していたためである。最終的には、その契約をランタイムテストへ置かない判断に伴って削除した。
...
この時点ではSkippedの1件が`ERROR-02`だった。その後の最終検証では`ERROR-02`を削除し、全体で成功12件、失敗0件、Skipped 30件となった。
...
5. 適切に証明できない契約をUnit testの責務にしないと決めた場合は、恒久的なSkipped計画を残さない。

## まとめ

テストコードも保守対象であり、処理を高度にするほど良いわけではない。Mojicaでは、reflectionによる生成経路検査と、仕様にある公開値の確認を別の問いへ分けた。現在確認できる6件は直接assertionで読みやすく固定し、未定義値を生成できないという構造制約は型システムとレビューの責務へ分離した。
